...
const Calendar = ({ calendarClose }) => {
  ...
  // ✅ FIX: Recalculate days on first load when dates come from Redux
  useEffect(() => {
  ...
  const handleDateTypeChange = (event, newType) => {
  ...
  return (
    <Box className="calendar-container" sx={{ p: 2, maxWidth: 400, mx: "auto" }}>
        ...
        >
          <ToggleButton
            ...
            sx={{
              ...
              py: 0.5,
              fontSize: "12px",
              "&.Mui-selected": { backgroundColor: "#d946ef", color: "white" },
            }}
          >
            Event Start Date
          </ToggleButton>
      ...
      <Box sx={{ textAlign: "center", mb: 2 }}>
        {selectedDates[0] && (
          <Typography>Start Date: {selectedDates[0].toLocaleDateString("en-GB")}</Typography>
        )}

        {selectedDates[1] && (
          <Typography>End Date: {selectedDates[1].toLocaleDateString("en-GB")}</Typography>
        )}

        {selectedDates[0] && selectedDates[1] && (
          <Typography sx={{ color: "#c026d3" }}>
            Number of Days: {numberOfDays}
          </Typography>
        )}
      </Box>
      ...
      <Box sx={{ display: "flex", justifyContent: "center" }}>
        <Button
          variant="contained"
          onClick={handleConfirm}
          sx={{ backgroundColor: "#c026d3", px: 4, py: 1, borderRadius: 8 }}
        >
          Confirm
        </Button>
      </Box>
