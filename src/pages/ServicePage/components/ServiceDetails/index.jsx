...
const ServiceDetails = () => {
  ...
  // ✅ Fetch services
  // Show the vendor-created services for this category (the same records the
  // admin lists under "Service List", limited to Approved) instead of querying
  // vendors by their profession — otherwise services whose vendor profession
  // doesn't match the category name go missing on the website.
  useEffect(() => {
    const getService = async () => {
      try {
        dispatch(setLoading(true));
        const res = await authService.getApprovedVendorServices();
        const allServices = res.data?.data || [];
        const wanted = (serviceName || "").trim().toLowerCase();
        const data = allServices.filter(
          (s) => (s.service_category || "").trim().toLowerCase() === wanted
        );
  ...
  // ✅ Helper to safely get vendor name (in case of different API shapes)
  ...
  // ✅ Combined filtering (Search + Reviews)
  useEffect(() => {
    let filtered = [...serviceDetails];

    // 🔍 Filter by search (vendor/shop name)
    if (searchQuery.trim()) {
      const query = searchQuery.trim().toLowerCase();
      filtered = filtered.filter((service) => {
        const vendorName = getVendorName(service).toLowerCase();
        const shopName = (service.shop_name || "").toLowerCase();
    ...
    if (selectedRatings.length > 0) {
      filtered = filtered.filter((service) => {
        const avg = calculateAverageRating(service.Reviews);
        // Show if avgRating >= smallest selected threshold
        const minSelected = Math.min(...selectedRatings);
        return avg >= minSelected;
      });
    }

    setFilteredServices(filtered);
    setCurrentPage(1);
  }, [searchQuery, selectedRatings, serviceDetails]);
            ...
            <Collapse in={openSections.categories}>
              <Box sx={{ mt: 3 }}>
                <TextField
                  ...
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="Type vendor or shop name..."
                  sx={{
                    mb: 2,
                    width: "100%",
                    "& .MuiInputBase-root": {
            ...
            getPaginatedData().map((item) => (
              <Grid item xs={12} sm={6} md={4} lg={3} key={item._id}>
                <Card
                  ...
                  onClick={() => handleServiceClick(item.vendor_id)}
                >
                  <CardMedia
                    component="img"
                    sx={{
                  ...
                  <CardContent sx={{ textAlign: "center" }}>
                    ...
                    <Typography
                      ...
                      sx={{ mb: 0.5 }}
                    >
                      Vendor: {getVendorName(item) || "N/A"}
                    </Typography>
                    {item.price != null && item.price !== "" && (
                      <Typography
                        variant="subtitle2"
                        sx={{ mb: 1, color: "#c026d3", fontWeight: 600 }}
                      >
                        ₹{item.price}
                      </Typography>
                    )}
